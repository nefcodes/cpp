...
class LinkedList {
					...
					public:
						...
						~LinkedList() { // destructor, we have to write a destructor bcz, yeah the default destructor 
										// when you write "delete myLinkedList" would deleted the linkedlist memory, but not nodes.

							node* temp = head;
							while (head != nullptr) {
								head = head->next;
								delete temp; // this is one that deletes the temp
								temp = head;
							}
